...
async function api(req,res,url) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/health') return json(res,200,{ok:true,version:'2.3.0'});
  ...
  if (req.method === 'GET' && url.pathname === '/api/export') {
    ...
    const d=JSON.stringify({version:2.3,config:publicConfig(),questions},null,2);
    res.writeHead(200,{'Content-Type':'application/json','Content-Disposition':'attachment; filename="foco-show-pack-v23.json"'});
    return res.end(d);
  }
...
server.listen(PORT,'0.0.0.0',()=>{
  console.log(`FOCO V2.3: http://localhost:${PORT}`);
